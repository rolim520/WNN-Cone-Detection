...
# --- Configurações Iniciais para o BRANCO ---
ESTADO = {
    'idx': 0,
    'h_min': 0, 'h_max': 179,    # Hue (Pode abranger tudo para branco)
    's_min': 0, 's_max': 50,     # Saturação (Tem que ser baixa, ausência de cor)
    'v_min': 180, 'v_max': 255,  # Valor/Brilho (Tem que ser alto, muita luminosidade)
...
def processar_e_exibir(*args):
    ...
    # 1. Máscara Única de Cor (Branco)
    limite_inferior = np.array([ESTADO['h_min'], ESTADO['s_min'], ESTADO['v_min']])
    limite_superior = np.array([ESTADO['h_max'], ESTADO['s_max'], ESTADO['v_max']])
    mask_cor = cv2.inRange(hsv_atual, limite_inferior, limite_superior)
    ...
    cv2.putText(img_display, "Original", (10, 30), fonte, 0.8, cor_texto, 2)
    cv2.putText(mask_bgr, "Mascara Branca", (10, 30), fonte, 0.8, cor_texto, 2)
    cv2.putText(resultado_bgr, "Resultado", (10, 30), fonte, 0.8, cor_texto, 2)
...
def imprimir_e_sair():
    print("\n--- Parâmetros Finais (Branco) ---")
    print(f"limite_inferior_branco = np.array([{ESTADO['h_min']}, {ESTADO['s_min']}, {ESTADO['v_min']}])")
    print(f"limite_superior_branco = np.array([{ESTADO['h_max']}, {ESTADO['s_max']}, {ESTADO['v_max']}])")
    print("\n## Morfologia")
...
root = tk.Tk()
root.title("Calibrador HSV - Detecção de Branco")
...
frame_h = tk.LabelFrame(frame_controles, text="Matiz / Hue (Geralmente 0-179 para branco)")
frame_h.pack(fill='x', pady=2)
criar_slider(frame_h, "H Min", 'h_min', 0, 179)
criar_slider(frame_h, "H Max", 'h_max', 0, 179)

frame_s = tk.LabelFrame(frame_controles, text="Saturação (Baixa para branco)")
frame_s.pack(fill='x', pady=2)
criar_slider(frame_s, "S Min", 's_min', 0, 255)
criar_slider(frame_s, "S Max", 's_max', 0, 255)

frame_v = tk.LabelFrame(frame_controles, text="Brilho / Value (Alto para branco)")
frame_v.pack(fill='x', pady=2)
criar_slider(frame_v, "V Min", 'v_min', 0, 255)
criar_slider(frame_v, "V Max", 'v_max', 0, 255)
