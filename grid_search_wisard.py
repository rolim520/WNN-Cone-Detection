...
def worker_avaliar_configuracao(params):
    modo, res, tupla, ign_zero = params
    
    # Recupera os dados pré-binarizados da memória global
    ...
    for rodada in range(NUM_EXECUCOES):
        ...
        # 4. Deleta o modelo treinado da RAM após o uso
        del modelo
        gc.collect()
...
if __name__ == "__main__":
    ...
    print("="*70)
    print(f" GRID SEARCH PARALELO DA WISARD ({NUM_EXECUCOES} Execuções por Setup)")
        ...
        for res in GRID_RESOLUCOES:
            for tupla in GRID_TUPLAS:
                for ign_zero in GRID_IGNORE_ZERO:
                    tarefas.append((modo, res, tupla, ign_zero))

    historico_resultados = []
    tempo_inicial = time.perf_counter()
    
    # A WiSARD clássica usa menos memória que a ClusWisard, mas ainda limitamos 
    # a 6 workers para evitar superlotação de RAM em resoluções grandes.
    ...
    with multiprocessing.Pool(processes=max_workers, maxtasksperchild=1) as pool:
        ...
        for res in pool.imap_unordered(worker_avaliar_configuracao, tarefas):
            ...
            cfg = f"Modo:{res['modo'][:3]}|Res:{res['res']:2d}|Tup:{res['tupla']:2d}|Ign0:{str(res['ign_zero'])[0]}"
